...
export function App(){
  ...
  return <main className="shell">
    <header><div className="brand"><span>A</span> ANURA</div><div className="header-actions"><InstallButton/><button className="icon-btn" onClick={logout} aria-label="Cerrar sesión"><LogOut size={19}/></button></div></header>
...
function InstallButton(){
  ...
  useEffect(()=>{const ready=(event:Event)=>{event.preventDefault();setPrompt(event as InstallPromptEvent)};window.addEventListener('beforeinstallprompt',ready);return()=>window.removeEventListener('beforeinstallprompt',ready)},[]);
  if(!mobile||standalone)return null;
  return <button className="install-btn" onClick={async()=>{if(prompt){await prompt.prompt();await prompt.userChoice;setPrompt(null)}else alert('En iPhone: pulsa Compartir y después “Añadir a pantalla de inicio”.')}}><Download size={16}/><span>Instalar</span></button>
}
