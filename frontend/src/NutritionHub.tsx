...
function TodayNutritionCard({meal,date,refresh,open,toggle}:{meal:TodayMeal;date:string;refresh:()=>Promise<void>;open:()=>void;toggle:()=>Promise<void>}){const [options,setOptions]=useState([] as MealOption[]),[optionError,setOptionError]=useState(""),[changing,setChanging]=useState(false),substituted=meal.status==="SUBSTITUTED",partial=meal.status==="PARTIAL",plannedCalories=Number(meal.planned_calories??0),fit=substituted?substitutionFit(meal):"";useEffect(()=>{setOptionError("");void nutritionApi.mealOptions(meal.planned_meal_id).then(setOptions).catch(()=>{setOptions([]);setOptionError("No se han podido cargar las opciones de esta comida.")})},[meal.planned_meal_id]);return <article className={`${meal.status!=="PENDING"?"completed":""} ${substituted?`substituted substitution-${fit}`:""}`}><button className="today-recipe-link" onClick={open}><small>{meal.meal_type}{substituted?` · HECHA · ${fit==="aligned"?"MUY AJUSTADA":fit==="close"?"AJUSTADA":"DIFERENTE AL PLAN"}`:partial?` · PARCIAL · ${meal.adherence_percent||0}%`:""}</small>{substituted?<><b>Sustituido por {meal.custom_name||"otra comida"}</b><em className="actual-meal-macros">Real: {Number(meal.calories||0).toFixed(0)} kcal · P {Number(meal.protein||0).toFixed(0)} · C {Number(meal.carbohydrates||0).toFixed(0)} · G {Number(meal.fat||0).toFixed(0)}{meal.actual_portion?` · ${meal.actual_portion}`:""}</em><span className="planned-meal-reference">Previsto: {meal.recipe||meal.meal_name} · {plannedCalories.toFixed(0)} kcal · diferencia {signedCalories(Number(meal.calories||0)-plannedCalories)}</span></>:<><b>{meal.recipe||meal.meal_name}</b><em>{Number(meal.calories||0).toFixed(0)} kcal · P {Number(meal.protein||0).toFixed(0)} · C {Number(meal.carbohydrates||0).toFixed(0)} · G {Number(meal.fat||0).toFixed(0)}</em></>}</button>{options.length>1&&meal.status==="PENDING"&&<select className="meal-option-select" value={meal.option_code||"DEFAULT"} disabled={changing} aria-label={`Opción para ${meal.meal_name}`} onChange={async e=>{setChanging(true);try{await nutritionApi.selectMealOption(meal.planned_meal_id,e.target.value,date);await refresh()}finally{setChanging(false)}}>{options.map(option=><option key={option.option_code} value={option.option_code}>{option.option_label} · {Number(option.calories).toFixed(0)} kcal</option>)}</select>}{optionError&&<small className="meal-option-error">{optionError}</small>}<button aria-label={meal.status==="PENDING"?`Completar ${meal.meal_name}`:`Deshacer ${meal.meal_name}`} title={meal.status==="PENDING"?"Marcar como hecha":"Deshacer registro"} onClick={()=>void toggle()}>{meal.status!=="PENDING"?"✓":"○"}</button></article>}
