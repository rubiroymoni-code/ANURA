...
function CookByDate({planId,recipes,open}:{planId?:string;recipes:Array<{id:string;name:string}>;open:(recipeId:string,mealId:string)=>void}) {
  ...
  const weekday=normalizeWeekday(new Intl.DateTimeFormat("es",{weekday:"long"}).format(new Date()));
  const todayDay=days.find(([,name])=>normalizeWeekday(name).startsWith(weekday.slice(0,3)))?.[0]??days[0]?.[0]??1;
