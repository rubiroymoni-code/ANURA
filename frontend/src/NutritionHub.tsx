...
function Shopping({ plans }: { plans: Array<{ id: string;name:string;status:string }> }) {
  ...
  const load=async(preferred?:string)=>{const next=await nutritionApi.shopping();setLists(next);const selected=next.find(x=>x.id===preferred)||next.find(x=>x.week_number===week);setListId(selected?.id||"");setItems(selected?await nutritionApi.items(selected.id):[]);window.dispatchEvent(new Event("anura:reminders-changed"))};
