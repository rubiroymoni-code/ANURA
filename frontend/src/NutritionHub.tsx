...
function Shopping({ plans }: { plans: Array<{ id: string;name:string;status:string }> }) {
  ...
  const [lists,setLists]=useState<Awaited<ReturnType<typeof nutritionApi.shopping>>>([]),[items,setItems]=useState<Awaited<ReturnType<typeof nutritionApi.items>>>([]),[week,setWeek]=useState(weekOptions[0].number),[listId,setListId]=useState(""),[error,setError]=useState(""),[planId,setPlanId]=useState(activePlan?.id||"");
  ...
  const generate=async()=>{if(!selectedPlan)return;setError("");try{const result=await nutritionApi.generateShopping(selectedPlan.id,week);await load(result.id)}catch(cause){if(cause instanceof Error&&cause.message.includes("modificada")&&confirm("La lista tiene cambios manuales. ¿Regenerarla igualmente?")){const result=await nutritionApi.generateShopping(selectedPlan.id,week,true);await load(result.id)}else setError(cause instanceof Error?cause.message:"No se pudo generar")}};
