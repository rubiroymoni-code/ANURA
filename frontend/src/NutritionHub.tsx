...
function CookByDate({planId,recipes,open}:{planId?:string;recipes:Array<{id:string;name:string}>;open:(recipeId:string,mealId:string)=>void}) {
  ...
  return <div className="cook-workspace"><BatchCooking planId={planId}/><section className="cook-today"><div className="food-pref-hero"><ChefHat/><span><small>{"COCINA DEL D\u00cdA"}</small><h3>{visibleDay===todayDay?"Preparaciones de hoy":`Preparaciones del ${days.find(([day])=>day===visibleDay)?.[1]?.toLowerCase()||`d\u00eda ${visibleDay}`}`}</h3><p>Elige para cada persona si esa comida es en casa o fuera.</p></span></div>{days.length>0&&<div className="plan-day-tabs">{days.map(([day,name])=><button key={day} className={visibleDay===day?"active":""} onClick={()=>setSelectedDay(day)}><b>{day===todayDay?"Hoy":name.slice(0,3)}</b><small>{"D\u00eda "}{day}</small></button>)}</div>}{loading?<div className="empty">Cargando comidas...</div>:dayMeals.length?dayMeals.map(meal=><CookFamilyMeal key={`${String(meal.user_id)}-${String(meal.planned_meal_id)}`} meal={meal} recipes={recipes} open={open} reload={reload}/>):<div className="empty">{"No hay comidas asignadas para este d\u00eda."}</div>}</section></div>;
}
function CookFamilyMeal({meal,recipes,open,reload}:{meal:Record<string,unknown>;recipes:Array<{id:string;name:string}>;open:(recipeId:string,mealId:string)=>void;reload:()=>Promise<void>}){
  const [options,setOptions]=useState([] as MealOption[]);
  const [changing,setChanging]=useState(false);
  useEffect(()=>{void nutritionApi.mealOptions(String(meal.planned_meal_id),String(meal.user_id)).then(setOptions).catch(()=>setOptions([]))},[meal.planned_meal_id,meal.user_id]);
  const recipe=recipes.find(item=>item.name.trim().toLocaleLowerCase("es")===String(meal.recipe).trim().toLocaleLowerCase("es"));
  const selectOption=async(optionCode:string)=>{setChanging(true);try{await nutritionApi.selectMealOption(String(meal.planned_meal_id),optionCode,String(meal.meal_date),String(meal.user_id));await reload()}finally{setChanging(false)}};
  return <article className="cook-family-meal"><button disabled={!recipe} onClick={()=>recipe&&open(recipe.id,String(meal.planned_meal_id))}><span><small>{String(meal.display_name)} \u00b7 {mealTypeLabel(String(meal.meal_type))}</small><b>{String(meal.recipe)}</b><em>{Number(meal.calories||0).toFixed(0)} kcal</em></span><strong>Ver reparto</strong></button>{options.length>1&&<label className="cook-location-choice"><span>{String(meal.display_name)}</span><select value={String(meal.option_code||"DEFAULT")} disabled={changing} onChange={event=>void selectOption(event.target.value)}>{options.map(option=><option key={option.option_code} value={option.option_code}>{option.option_label}</option>)}</select></label>}</article>;
}
  ...
  return <div className="shopping-view">
    ...
    {!listId?<div className="empty">Genera la lista de la semana desde tu plan activo.</div>:<>
      ...
      {categories.map(category=><section className="shopping-category" key={category}><h4>{categoryLabel(category)}</h4>{items.filter(i=>(i.category||"OTHER")===category).map(i=>{const covered=i.quantity<=0&&i.pantry_used>=i.required_quantity;return <article className={`shopping-item ${i.purchased?"purchased":""} ${covered?"covered":""}`} key={i.id}>
        {covered?<span className="shopping-check">âœ“</span>:<button className="shopping-check" onClick={async event=>{const input=event.currentTarget.parentElement?.querySelector<HTMLInputElement>("input");if(input)await nutritionApi.shoppingQuantity(i.id,Number(input.value));await nutritionApi.toggle(i.id);await refreshItems()}}>{i.purchased?"âœ“":"â—‹"}</button>}<div><b>{i.name}</b><small>{covered?`Cubierto por despensa · ${displayQuantity(i.pantry_used,i.unit)} disponibles`:`Necesario: ${displayQuantity(i.required_quantity,i.unit)}${i.pantry_used>0?` · Ya disponible: ${displayQuantity(i.pantry_used,i.unit)}`:""}`}</small></div>{covered?<strong className="pantry-covered">No comprar</strong>:<label>Comprado<input type="number" min="0" step="0.001" defaultValue={i.quantity}/><span>{i.unit}</span></label>}
...
function PantryAddForm({saved}:{saved:(item:{ingredient_id:string;name:string;category:string;quantity:number;unit:string})=>Promise<void>}){
 ...
 return <form className="pantry-add" onSubmit={async event=>{event.preventDefault();if(!name.trim()||saving)return;const data=new FormData(event.currentTarget),enteredQuantity=Number(data.get("quantity")),normalizedUnit=unit==="kg"?"g":unit==="l"?"ml":unit,normalizedQuantity=enteredQuantity*(unit==="kg"||unit==="l"?1000:1);setSaving(true);try{const result=await nutritionApi.addPantry({name:name.trim(),category,quantity:enteredQuantity,unit});const added={ingredient_id:result.ingredientId,name:result.name,category,quantity:normalizedQuantity,unit:normalizedUnit};setName("");setCategory("PANTRY");setUnit("g");setSuggestions([]);await saved(added)}finally{setSaving(false)}}}>
  ...
  <button className="primary" disabled={saving}>{saving?"Guardandoâ€¦":"Añadir"}</button>
 </form>
}
