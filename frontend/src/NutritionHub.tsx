...
export function NutritionHub({ onClose,onRegisterMeal,onAddMeal,initialSection="home",initialRecipeMeal,onInitialRecipeOpened }: { onClose: () => void;onRegisterMeal?:(meal:TodayMeal)=>void;onAddMeal?:()=>void;initialSection?:NutritionSection;initialRecipeMeal?:TodayMeal|null;onInitialRecipeOpened?:()=>void }) {
  ...
  const previousSection=sectionHistory.at(-1),sectionName=(value?:NutritionSection)=>({home:"Hoy",cook:"Cocina",preferences:"Preferencias",household:"Unidad domÃ©stica",import:"ImportaciÃ³n",shopping:"Compra",supplements:"Suplementos",travel:"Modo viaje",plan:"Plan",recipe:"Receta",manage:"Planes y gestiÃ³n"} as Record<NutritionSection,string>)[value||"home"];
  ...
  useEffect(() => {
    ...
    void Promise.allSettled([
      ...
      if (rejected?.status === "rejected") setLoadError(rejected.reason instanceof Error ? rejected.reason.message : "No se pudo cargar nutriciÃ³n");
    });
  }, []);
    ...
    <div className="overlay">
      <section className="modal nutrition-hub">
        <div className="modal-head">
          <div>
            <small>{households.length?"NUTRICIÃ“N Y HOGAR":"MI NUTRICIÃ“N"}</small>
            <h2>
              {section === "home"
                ? "Hoy"
                : section === "cook"
        ...
        {expiry.urgent&&<section className="nutrition-expiry-warning" role="alert"><CalendarDays/><span><b>{expiry.expired?"Plan caducado":"Actualiza tu plan"}</b><small>{expiry.message} Genera el prompt con tu progreso e importa la siguiente versiÃ³n para mantener comidas, macros y compra al dÃ­a.</small></span></section>}
        {section === "home" && (
          <>
            {dashboard&&<section className={`nutrition-overview ${balanceExpanded?"expanded":""}`}><button className="nutrition-overview-summary" onClick={()=>setBalanceExpanded(value=>!value)}><span><small>BALANCE DE HOY</small><b>{Number(dashboard.consumed.calories||0).toFixed(0)} / {Number(dashboard.target.calories||dashboard.planned.calories||0).toFixed(0)} kcal</b><em>{activePlan?`${activePlan.name} Â· versiÃ³n ${activePlan.version}`:"Sin plan activo"}</em></span><strong>{Math.round(Number(dashboard.target.calories||dashboard.planned.calories||0)?Number(dashboard.consumed.calories||0)/Number(dashboard.target.calories||dashboard.planned.calories||1)*100:0)}%</strong><ChevronDown/></button>{balanceExpanded&&<div className="nutrition-overview-detail"><NutritionBalance data={dashboard}/></div>}</section>}
            <section className="nutrition-today"><div><small>{mealDate===new Date().toLocaleDateString("en-CA")?"HOY":"OTRO DÃA"}</small><h3>Lo que te toca comer</h3><p>TambiÃ©n puedes completar una comida pendiente de un dÃ­a anterior.</p><label className="meal-day-picker">Ver dÃ­a<input type="date" value={mealDate} max={new Date().toLocaleDateString("en-CA")} onChange={event=>{setMealDate(event.target.value);void reloadMeals(event.target.value);void nutritionApi.travelToday(event.target.value).then(setTravelToday)}}/></label></div>{travelToday.id&&<article className="travel-today-card"><Luggage/><span><small>{travelToday.title} Â· {travelToday.plan_label||"DÃ­a flexible"}</small><b>{travelToday.guidance||"Come con flexibilidad y registra solo lo que te resulte Ãºtil."}</b><em>No genera compra ni penaliza la adherencia.</em></span></article>}{todayMeals.length?todayMeals.map(meal=><TodayNutritionCard key={meal.planned_meal_id} meal={meal} date={mealDate} refresh={()=>reloadMeals()} open={()=>{const recipe=recipes.find(r=>r.name.trim().toLowerCase()===meal.recipe.trim().toLowerCase());if(recipe){setSelectedRecipe(recipe.id);setSelectedMeal(meal.planned_meal_id);navigate("recipe")}}} toggle={async()=>{if(meal.status==="PENDING")await nutritionApi.completeToday(meal.planned_meal_id,mealDate);else await nutritionApi.undoToday(meal.planned_meal_id,mealDate);await reloadMeals();if(mealDate===new Date().toLocaleDateString("en-CA"))setDashboard(await nutritionApi.dashboard())}}/>):!travelToday.id&&<p>No hay comidas asignadas para ese dÃ­a en el plan activo.</p>}{onAddMeal&&<button className="nutrition-add-meal" onClick={onAddMeal}><Plus/>Registrar otra comida</button>}</section>
          </>
        )}
        {section === "manage" && <div className="nutrition-tools plan-management"><div className="nutrition-menu">
          <button className="workout-import-action" onClick={() => navigate("import")}><FileUp/><span><b>Importar dieta</b><small>Sube una nueva planificaciÃ³n o versiÃ³n</small></span></button>
          ...
          <section className="managed-plan-section managed-settings-section"><header><small>CONFIGURACIÃ“N</small><b>Preferencias y hogar</b></header><div className="nutrition-menu">
            <button onClick={() => navigate("household")}><Users/><b>Mi unidad domÃ©stica</b><span>{households[0]?.name || "Crear o aceptar invitaciÃ³n"}</span></button>
            <button onClick={() => navigate("preferences")}><SlidersHorizontal/><b>Preferencias alimentarias</b><span>Gustos, exclusiones y planificaciÃ³n</span></button>
        ...
        {section !== "home" && (
          <button className="text-btn" onClick={goBack}>
            â† Volver a {sectionName(previousSection)}
          </button>
        )}
        </div>
      </section>
...
function nutritionPlanExpiry(value?:string,replacementReady=false){if(!value||replacementReady)return{urgent:false,expired:false,message:""};const days=Math.ceil((new Date(`${value}T23:59:59`).getTime()-Date.now())/86400000);return{urgent:days<=1,expired:days<0,message:days<0?"El plan ya ha terminado.":days===0?"El plan termina hoy.":"Al plan le queda un dÃ­a."}}
function NutritionPlanManagement({plans,open,refresh}:{plans:Array<{id:string;name:string;version:number;status:string;valid_from?:string;valid_until?:string}>;open:(id:string)=>void;refresh:()=>Promise<void>}){
 ...
 const PlanCard=({plan}:{plan:(typeof plans)[number]})=>{
  const remove=async()=>{if(!confirm("Â¿Eliminar este plan nutricional? Se borrarÃ¡n sus dÃ­as y comidas planificadas. Esta acciÃ³n no se puede deshacer."))return;await nutritionApi.deletePlan(plan.id);await refresh()};
  return <article><button className="managed-plan-open" onClick={()=>open(plan.id)}><span><small>{plan.status==="ACTIVE"?"Plan actual":plan.status==="DRAFT"?"PrÃ³ximo plan":"Plan archivado"}</small><b>{plan.name}</b><em>VersiÃ³n {plan.version}{plan.valid_from?` Â· Desde ${plan.valid_from}`:""}{plan.valid_until?` Â· Hasta ${plan.valid_until}`:""}</em></span></button><div className="managed-plan-actions">{plan.status!=="ACTIVE"&&<button className="managed-plan-activate" onClick={async()=>{await nutritionApi.activate(plan.id);await refresh()}}>Activar</button>}<button className="managed-plan-delete" aria-label={`Eliminar ${plan.name}`} onClick={()=>void remove()}><Trash2/><span>Eliminar</span></button></div></article>
 };
 return <><section className="managed-plan-section"><header><small>PLAN ACTUAL</small><b>Lo que se aplica hoy</b></header>{active?<PlanCard plan={active}/>:<p className="managed-plan-empty">No hay un plan activo.</p>}</section>{drafts.length>0&&<section className="managed-plan-section"><header><small>PRÃ“XIMO PLAN</small><b>Preparado para activarse</b></header>{drafts.map(plan=><PlanCard key={plan.id} plan={plan}/>)}</section>}{past.length>0&&<details className="managed-plan-history"><summary>Historial de planes ({past.length})</summary><div>{past.map(plan=><PlanCard key={plan.id} plan={plan}/>)}</div></details>}</>
}
      ...
      {h ? (
        <>
          ...
          {h.role === "OWNER" && (
            <>
              <label>
                Email de la persona (opcional; no se envÃ­a)
                <input
                  value={email}
                ...
                onClick={async () => {
                  ...
                  setMessage(
                    r.recipientStatus === "NEW_USER"
                      ? `Ese email todavÃ­a no tiene cuenta. Se ha preparado una invitaciÃ³n para registrarse en ANURA${r.deliveryStatus === "SENT" ? " y se ha enviado por correo" : ""}.`
                      : r.recipientStatus === "REGISTERED_USER"
                        ? `Usuario encontrado${r.deliveryStatus === "SENT" ? ": invitaciÃ³n enviada por correo" : ""}.`
                        : "CÃ³digo listo para compartir.",
                  );
                }}
              >
                {email ? "Crear invitaciÃ³n para este email" : "Generar cÃ³digo para compartir"}
              </button>
              {generated && (
                <div className="invite-code">
                  <small>CÃ“DIGO TEMPORAL Â· 24 HORAS</small>
                  ...
                  <div className="invite-actions">
                    ...
                    <a
                      href={`https://wa.me/?text=${encodeURIComponent(`Ãšnete a mi unidad domÃ©stica en ANURA con este cÃ³digo: ${generated}`)}`}
                      target="_blank"
                      rel="noreferrer"
                    >
              ...
              {message && <p className="form-note">{message} ANURA todavÃ­a no envÃ­a emails.</p>}
            </>
          )}
        </>
      ) : (
        <>
          ...
          >
            Crear unidad domÃ©stica
          </button>
          {generated && (
            <div className="invite-code">
              <small>CÃ“DIGO DE TU NUEVA UNIDAD Â· 24 HORAS</small>
              <strong>{generated}</strong>
              <div className="invite-actions">
                <button type="button" onClick={() => void navigator.clipboard.writeText(generated)}>
                  <Copy /> Copiar
          ...
          <div className="or">o aceptar invitaciÃ³n</div>
          <label>
            CÃ³digo
      ...
      {p && (
        <div className="preview-card">
          <span className={p.confirmable ? "valid" : "invalid"}>
            {p.confirmable ? "Archivo vÃ¡lido" : "Corrige los errores"}
          </span>
          ...
          {p.issues.map((i, n) => (
            <details key={n}>
              <summary>
                Fila {i.row || "â€”"} Â· {i.column || "archivo"}
              </summary>
              <p>{i.message}</p>
            </details>
          ))}
          ...
          try {
            ...
            setError(
              cause instanceof Error
                ? cause.message
                : "No se pudo completar la importaciÃ³n",
            );
          } finally {
            setBusy(false);
          }
...
function CookToday({planId,recipes,open}:{planId?:string;recipes:Array<{id:string;name:string}>;open:(recipeId:string,mealId:string)=>void}){const [rows,setRows]=useState<Array<Record<string,unknown>>>([]),[selectedDay,setSelectedDay]=useState(new Date().getDay()||7),[loading,setLoading]=useState(false);const currentUser=JSON.parse(localStorage.getItem("anura-user")||"{}");useEffect(()=>{if(!planId){setRows([]);return}setLoading(true);void nutritionApi.week(planId).then(setRows).finally(()=>setLoading(false))},[planId]);const todayDay=new Date().getDay()||7,mine=rows.filter(row=>String(row.user_id)===String(currentUser.id)),days=[...new Map(mine.map(row=>[Number(row.day_number),String(row.day_name||`DÃ­a ${row.day_number}`)])).entries()].sort((a,b)=>a[0]-b[0]),dayMeals=mine.filter(row=>Number(row.day_number)===selectedDay).sort((a,b)=>Number(a.meal_order||0)-Number(b.meal_order||0));return <div className="cook-workspace"><BatchCooking planId={planId}/><section className="cook-today"><div className="food-pref-hero"><ChefHat/><span><small>COCINA DEL DÃA</small><h3>{selectedDay===todayDay?"Preparaciones de hoy":`Preparaciones del ${days.find(([day])=>day===selectedDay)?.[1]?.toLowerCase()||`dÃ­a ${selectedDay}`}`}</h3><p>Abre una receta para ver el reparto exacto de cada comida.</p></span></div>{days.length>0&&<div className="plan-day-tabs">{days.map(([day,name])=><button key={day} className={selectedDay===day?"active":""} onClick={()=>setSelectedDay(day)}><b>{day===todayDay?"Hoy":name.slice(0,3)}</b><small>DÃ­a {day}</small></button>)}</div>}{loading?<div className="empty">Cargando comidasâ€¦</div>:dayMeals.length?dayMeals.map(meal=>{const recipe=recipes.find(item=>item.name.trim().toLocaleLowerCase("es")===String(meal.recipe).trim().toLocaleLowerCase("es"));return <button key={String(meal.planned_meal_id)} disabled={!recipe} onClick={()=>recipe&&open(recipe.id,String(meal.planned_meal_id))}><span><small>{mealTypeLabel(String(meal.meal_type))}</small><b>{String(meal.recipe)}</b><em>{Number(meal.calories||0).toFixed(0)} kcal para ti</em></span><strong>Ver reparto â†’</strong></button>}):<div className="empty">No hay preparaciones asignadas para este dÃ­a.</div>}</section></div>}
...
function CookByDate({planId,recipes,open}:{planId?:string;recipes:Array<{id:string;name:string}>;open:(recipeId:string,mealId:string)=>void}) {
  ...
  const days=[...new Map(mine.map(row=>[Number(row.day_number),String(row.day_name||`DÃ­a ${row.day_number}`)])).entries()].sort((a,b)=>a[0]-b[0]);
  ...
  return <div className="cook-workspace"><BatchCooking planId={planId}/><section className="cook-today"><div className="food-pref-hero"><ChefHat/><span><small>COCINA DEL DÃA</small><h3>{visibleDay===todayDay?"Preparaciones de hoy":`Preparaciones del ${days.find(([day])=>day===visibleDay)?.[1]?.toLowerCase()||`dÃ­a ${visibleDay}`}`}</h3><p>Abre una receta para ver el reparto exacto de cada comida.</p></span></div>{days.length>0&&<div className="plan-day-tabs">{days.map(([day,name])=><button key={day} className={visibleDay===day?"active":""} onClick={()=>setSelectedDay(day)}><b>{day===todayDay?"Hoy":name.slice(0,3)}</b><small>DÃ­a {day}</small></button>)}</div>}{loading?<div className="empty">Cargando comidasâ€¦</div>:dayMeals.length?dayMeals.map(meal=>{const recipe=recipes.find(item=>item.name.trim().toLocaleLowerCase("es")===String(meal.recipe).trim().toLocaleLowerCase("es"));return <button key={String(meal.planned_meal_id)} disabled={!recipe} onClick={()=>recipe&&open(recipe.id,String(meal.planned_meal_id))}><span><small>{mealTypeLabel(String(meal.meal_type))}</small><b>{String(meal.recipe)}</b><em>{Number(meal.calories||0).toFixed(0)} kcal para ti</em></span><strong>Ver reparto â†’</strong></button>}):<div className="empty">No hay preparaciones asignadas para este dÃ­a.</div>}</section></div>
}
...
function BatchCooking({planId}:{planId?:string}){const [rows,setRows]=useState<PrepRow[]>([]),[period,setPeriod]=useState<"remaining"|"week">("week"),[open,setOpen]=useState(false),[loading,setLoading]=useState(false);useEffect(()=>{if(!planId){setRows([]);return}setLoading(true);void nutritionApi.week(planId).then(value=>setRows(value as unknown as PrepRow[])).finally(()=>setLoading(false))},[planId]);const today=new Date().getDay()||7,selected=rows.filter(row=>period==="week"||Number(row.day_number)>=today),ingredients=batchIngredients(selected),containers=batchContainers(selected);return <section className={`batch-cooking ${open?"open":""}`}><button className="batch-cooking-head" onClick={()=>setOpen(value=>!value)}><span className="batch-cooking-icon"><ChefHat/></span><span><small>PREPARAR DE UNA VEZ</small><b>Organiza las raciones de la unidad</b><em>{loading?"Calculandoâ€¦":rows.length?`${ingredients.prep.length} preparaciones Â· ${containers.length} recipientes`:"Necesitas un plan activo"}</em></span><strong>{open?"Cerrar":"Preparar semana"}</strong></button>{open&&<div className="batch-cooking-body"><div className="batch-period"><button className={period==="week"?"active":""} onClick={()=>setPeriod("week")}>Toda la semana</button><button className={period==="remaining"?"active":""} onClick={()=>setPeriod("remaining")}>Desde hoy</button></div><p className="batch-intro"><b>Orden recomendado:</b> pesa en crudo, cocina cada ingrediente en una tanda, vuelve a pesar si quieres mÃ¡xima precisiÃ³n y reparte por recipientes siguiendo las tarjetas.</p>{ingredients.prep.length?<><h3>1. Cocina estas cantidades</h3><div className="batch-totals">{ingredients.prep.map(item=>{const factor=cookingFactor(item.name,item.unit),cooked=factor?item.quantity*factor:null,people=batchPeople(selected,item);return <article key={`${item.name}-${item.unit}`}><header><span><b>{item.name}</b><small>Total conjunto Â· crudo/seco</small></span><strong>{batchQuantity(item.quantity,item.unit)}</strong></header>{cooked!=null&&<p>Rendimiento aproximado: <b>â‰ˆ {batchQuantity(cooked,"g")} cocinado</b></p>}<div>{people.map(person=><span key={person.name}><b>{person.name}</b><small>{batchQuantity(person.quantity,item.unit)}{factor?` â†’ â‰ˆ ${batchQuantity(person.quantity*factor,"g")}`:""}</small></span>)}</div></article>})}</div><h3>2. Reparte y etiqueta</h3><div className="batch-containers">{containers.map(container=><article key={container.key}><header><span><small>{container.day} Â· {mealTypeLabel(container.mealType)}</small><b>{container.person}</b><em>{container.recipe}</em></span><strong>RECIPIENTE</strong></header><div>{container.ingredients.map(item=>{const factor=cookingFactor(item.name,item.unit);return <span key={`${item.name}-${item.unit}`}><b>{item.name}</b><small>{factor?`â‰ˆ ${batchQuantity(item.quantity*factor,"g")} cocinado`:batchQuantity(item.quantity,item.unit)}</small></span>})}</div></article>)}</div></>:<div className="empty">No hay ingredientes adecuados para preparar por tandas en este periodo.</div>}{ingredients.fresh.length>0&&<details className="batch-fresh"><summary>Preparar al momento ({ingredients.fresh.length})</summary><p>Fruta, lÃ¡cteos, pan, aguacate, aliÃ±os y otros alimentos que suelen conservarse mejor sin montar con antelaciÃ³n.</p><div>{ingredients.fresh.map(item=><span key={`${item.name}-${item.unit}`}><b>{item.name}</b><small>{batchQuantity(item.quantity,item.unit)} para el periodo</small></span>)}</div></details>}<p className="batch-safety"><b>ConservaciÃ³n prÃ¡ctica:</b> deja enfriar antes de cerrar, guarda en frÃ­o y congela las raciones que no consumirÃ¡s en 3 dÃ­as. Las equivalencias cocinadas son aproximadas.</p></div>}</section>}
  ...
  return (
    <div>
      ...
      {portions.length>0&&<section className="recipe-person-portions">
        <div><small>CANTIDADES PARA ESTA COMIDA</small><h3>QuÃ© corresponde a cada persona</h3></div>
        {portions.map(person=><PortionCard key={String(person.user_id)} person={person}/>)}
        <article className="recipe-combined"><header><b>Total para cocinar Â· crudo/seco</b><span>{portions.reduce((sum,p)=>sum+Number(p.quantity||0),0).toFixed(0)} g</span></header><div className="portion-ingredients combined">{combinedIngredients(portions).map((ingredient,index)=><span key={`${ingredient.name}-${index}`}><b>{ingredient.name}</b><em>{ingredientQuantityLabel(ingredient.name,ingredient.quantity,ingredient.unit)}</em></span>)}</div></article>
        <p className="cooked-portions-note"><b>CÃ³mo usar el reparto cocinado</b> Cocina el total conjunto indicado arriba, pesa el alimento ya preparado y sirve aproximadamente la cantidad cocinada mostrada para cada persona. Son equivalencias medias: el agua, el mÃ©todo y el punto de cocciÃ³n pueden variar el peso.</p>
      </section>}
      {portions.length===0&&rows.map((r, n) => (
        <div className="nutrition-row" key={n}>
          <span>
            ...
            <small>
              {String(r.quantity)} {String(r.unit)} Â·{" "}
              {Number(r.protein_100 || 0)} g proteÃ­na/100
            </small>
          </span>
        </div>
      ))}
...
function PlanView({ id,status,onDeleted }: { id: string;status:string;onDeleted:()=>void }) {
  ...
  const days=[...new Map([...mine.map(row=>[Number(row.day_number),String(row.day_name||`DÃ­a ${row.day_number}`)] as [number,string]),...travelDays.map(day=>[isoDay(day.travel_date),weekdayName(day.travel_date)] as [number,string])]).entries()].sort((a,b)=>a[0]-b[0]);
  ...
  return (
    <div className="plan-agenda">
      <div className="plan-day-tabs">{days.map(([day,name])=><button key={day} className={selectedDay===day?"active":""} onClick={()=>setSelectedDay(day)}><b>{day===todayDay?"Hoy":name.slice(0,3)}</b><small>DÃ­a {day}</small></button>)}<button className={selectedDay===0?"active":""} onClick={()=>setSelectedDay(0)}><b>Todo</b><small>semana</small></button></div>
      <div className="plan-agenda-intro"><CalendarDays/><span><b>{selectedDay===todayDay?"Tu menÃº de hoy":selectedDay===0?"Tu semana completa":`Tu menÃº del dÃ­a ${selectedDay}`}</b><small>Para cantidades conjuntas y reparto abre Cocina.</small></span></div>
      {!dayRows.length&&!visibleTravel.length&&<div className="empty">No tienes comidas asignadas para este dÃ­a.</div>}
      ...
      <div className="plan-actions"><span className={status==="ACTIVE"?"active":""}>{status==="ACTIVE"?"â— Plan activo":nutritionPlanStatusLabel[status||""]||status||"Plan"}</span>{status!=="ACTIVE"&&<button className="primary" onClick={async()=>{await nutritionApi.activate(id);location.reload()}}>Activar este plan</button>}<button className="danger" onClick={async()=>{if(confirm("Â¿Eliminar este plan nutricional? Se borrarÃ¡n sus dÃ­as y comidas planificadas. Esta acciÃ³n no se puede deshacer.")){await nutritionApi.deletePlan(id);onDeleted()}}}>Eliminar plan</button></div>
    </div>
  );
}
...
function TravelPlanDay({day,showDay}:{day:{title:string;travel_date:string;plan_label:string;guidance:string};showDay:boolean}){return <article className="plan-travel-day"><span><Luggage/><small>{showDay?`${weekdayName(day.travel_date)} Â· `:""}{day.title}</small></span><h3>{day.plan_label||"Comer fuera"}</h3><p>{day.guidance}</p><footer><b>DÃ­a flexible planificado</b><em>Sin compra Â· fuera de adherencia</em></footer></article>}

function PlanMealCard({meal,dayVisible,open,toggle}:{meal:Record<string,unknown>;dayVisible:boolean;open:boolean;toggle:()=>void}){const ingredients=ingredientRows(meal.ingredients);return <article className={`plan-meal-card ${open?"open":""}`}><button className="plan-meal-summary" onClick={toggle} aria-expanded={open}><span className="agenda-time"><small>{mealTypeLabel(String(meal.meal_type))}</small><b>{String(meal.meal_name)}</b><em>{String(meal.recipe)}</em></span><span className="agenda-macros"><b>{Number(meal.calories||0).toFixed(0)} kcal</b><small>P {Number(meal.protein||0).toFixed(0)} Â· C {Number(meal.carbohydrates||0).toFixed(0)} Â· G {Number(meal.fat||0).toFixed(0)}</small></span><ChevronDown/>{dayVisible&&<i>DÃ­a {String(meal.day_number)}</i>}</button>{open&&<div className="plan-meal-ingredients"><small>INGREDIENTES Â· TU CANTIDAD</small>{ingredients.length?<div>{ingredients.map((ingredient,index)=><span key={`${ingredient.name}-${index}`}><b>{ingredient.name}</b><em>{displayQuantity(ingredient.quantity,ingredient.unit)}</em></span>)}</div>:<p>No hay ingredientes detallados para esta comida.</p>}<p>Para ver el total conjunto y el reparto de cada persona, abre <b>Cocina</b>.</p></div>}</article>}
...
function PortionCard({person}:{person:Record<string,unknown>}){const ingredients=ingredientRows(person.ingredients),cooked=cookedIngredientRows(ingredients);return <article><header><b>{String(person.display_name)}</b><span>{Number(person.quantity||0).toFixed(0)} g Â· {Number(person.calories||0).toFixed(0)} kcal</span></header><div className="portion-ingredients">{ingredients.map((ingredient,index)=><span key={`${ingredient.name}-${index}`}><b>{ingredient.name}</b><em>{ingredientQuantityLabel(ingredient.name,ingredient.quantity,ingredient.unit)}</em></span>)}</div>{cooked.length>0&&<div className="cooked-portion"><small>PARA SERVIR Â· PESO YA COCINADO</small><div className="portion-ingredients cooked">{cooked.map((ingredient,index)=><span key={`${ingredient.name}-${index}`}><b>{ingredient.name}</b><em>â‰ˆ {ingredient.quantity.toFixed(0)} g</em></span>)}</div></div>}</article>}
function ingredientQuantityLabel(name:string,quantity:number,unit:string){const normalized=name.toLocaleLowerCase("es"),grams=/^mg$/i.test(unit)?quantity/1000:/^kg$/i.test(unit)?quantity*1000:/^g(r|ramo|ramos)?$/i.test(unit)?quantity:null;if(grams!=null&&/(^|\s)huevos?(\s+entero)?(\s|$)/.test(normalized)){const raw=grams/60,rounded=Math.abs(raw-Math.round(raw))<.12?Math.round(raw):Math.abs(raw*2-Math.round(raw*2))<.15?Math.round(raw*2)/2:Math.round(raw*10)/10;return `â‰ˆ ${rounded.toLocaleString("es")} ${rounded===1?"huevo":"huevos"} Â· ${Math.round(grams)} g`}return `${Number(quantity).toFixed(0)} ${unit}`}
...
function cookingFactor(name:string,unit:string){if(!/^g(r|ramo|ramos)?$/i.test((unit||"g").trim()))return null;const value=name.toLocaleLowerCase("es");if(/cocid|cocinad|asado|plancha|hervid|preparad/.test(value))return null;if(/arroz/.test(value))return 2.8;if(/pasta|macarr|espaguet|tallar/.test(value))return 2.4;if(/quinoa/.test(value))return 3;if(/cusc[uÃº]s|couscous/.test(value))return 2.5;if(/lentej/.test(value))return 2.5;if(/garbanz/.test(value))return 2.4;if(/alubia|jud[iÃ­]a|frijol/.test(value))return 2.5;if(/avena/.test(value))return 3;if(/pollo|pavo/.test(value))return .75;if(/ternera|vacuno|cerdo/.test(value))return .72;if(/salm[oÃ³]n|merluza|at[uÃº]n|bacalao|pescado/.test(value))return .8;if(/br[oÃ³]coli|coliflor|calabac[iÃ­]n|berenjena|verdura/.test(value))return .85;return null}
...
function Shopping({ plans }: { plans: Array<{ id: string;name:string;status:string }> }) {
  ...
  const generate=async()=>{if(!selectedPlan||generating)return;setGenerating(true);setError("");try{const result=await nutritionApi.generateShopping(selectedPlan.id,week);await load(result.id);setFiltersOpen(false)}catch(cause){if(cause instanceof Error&&cause.message.includes("modificada")&&confirm("La lista tiene cambios manuales. Â¿Regenerarla igualmente?")){const result=await nutritionApi.generateShopping(selectedPlan.id,week,true);await load(result.id)}else setError(cause instanceof Error?cause.message:"No se pudo generar")}finally{setGenerating(false)}};
  ...
  return <div className="shopping-view">
    ...
    {!listId?<div className="empty">Genera la lista de la semana desde tu plan activo.</div>:<>
      ...
      {categories.map(category=><section className="shopping-category" key={category}><h4>{categoryLabel(category)}</h4>{items.filter(i=>(i.category||"OTHER")===category).map(i=>{const covered=i.quantity<=0&&i.pantry_used>=i.required_quantity;return <article className={`shopping-item ${i.purchased?"purchased":""} ${covered?"covered":""}`} key={i.id}>
        {covered?<span className="shopping-check">âœ“</span>:<button className="shopping-check" onClick={async event=>{const input=event.currentTarget.parentElement?.querySelector<HTMLInputElement>("input");if(input)await nutritionApi.shoppingQuantity(i.id,Number(input.value));await nutritionApi.toggle(i.id);await refreshItems()}}>{i.purchased?"âœ“":"â—‹"}</button>}<div><b>{i.name}</b><small>{covered?`Cubierto por despensa Â· ${displayQuantity(i.pantry_used,i.unit)} disponibles`:`Necesario: ${displayQuantity(i.required_quantity,i.unit)}${i.pantry_used>0?` Â· Ya disponible: ${displayQuantity(i.pantry_used,i.unit)}`:""}`}</small></div>{covered?<strong className="pantry-covered">No comprar</strong>:<label>Comprado<input type="number" min="0" step="0.001" defaultValue={i.quantity}/><span>{i.unit}</span></label>}
        {i.manual&&<button type="button" className="shopping-item-delete" aria-label={`Eliminar ${i.name}`} onClick={async()=>{if(confirm(`Â¿Eliminar ${i.name} de la lista?`)){await nutritionApi.deleteShoppingItem(i.id);await refreshItems()}}}><Trash2/>Eliminar</button>}
      </article>})}</section>)}
      <form className="shopping-add" onSubmit={async e=>{e.preventDefault();const f=new FormData(e.currentTarget);await nutritionApi.addShoppingItem(listId,{name:String(f.get("name")),category:String(f.get("category")),quantity:Number(f.get("quantity")),unit:String(f.get("unit"))});e.currentTarget.reset();await refreshItems()}}><b>AÃ±adir a la casa</b><input name="name" required placeholder="Producto"/><select name="category"><option value="OTHER">Otros</option><option value="PANTRY">Despensa</option><option value="FRUIT_VEGETABLES">Fruta y verdura</option><option value="MEAT_FISH">Carnes y pescados</option><option value="DAIRY">LÃ¡cteos</option></select><input name="quantity" required type="number" min="0" step="0.001" placeholder="Cantidad"/><UnitSelect/><button className="primary">AÃ±adir</button></form>
    </>}
  </div>;
}
function categoryLabel(value:string){return ({FRUIT_VEGETABLES:"Fruta y verdura",FRUTA:"Fruta",VERDURA:"Verdura",MEAT_FISH:"Carnes y pescados",PROTEINA:"ProteÃ­nas",EGGS:"Huevos",HUEVO:"Huevos",DAIRY:"LÃ¡cteos",LACTEO:"LÃ¡cteos",CEREALS_LEGUMES:"Cereales y legumbres",CEREAL:"Cereales",LEGUMBRE:"Legumbres",FROZEN:"Congelados",PANTRY:"Despensa",DESPENSA:"Despensa",DRINKS:"Bebidas",BEBIDA:"Bebidas",FRUTO_SECO:"Frutos secos",OTHER:"Otros"} as Record<string,string>)[value]||value}
...
function PantryAddForm({saved}:{saved:(item:{ingredient_id:string;name:string;category:string;quantity:number;unit:string})=>Promise<void>}){
 ...
 return <form className="pantry-add" onSubmit={async event=>{event.preventDefault();if(!name.trim()||saving)return;const data=new FormData(event.currentTarget),enteredQuantity=Number(data.get("quantity")),normalizedUnit=unit==="kg"?"g":unit==="l"?"ml":unit,normalizedQuantity=enteredQuantity*(unit==="kg"||unit==="l"?1000:1);setSaving(true);try{const result=await nutritionApi.addPantry({name:name.trim(),category,quantity:enteredQuantity,unit});const added={ingredient_id:result.ingredientId,name:result.name,category,quantity:normalizedQuantity,unit:normalizedUnit};setName("");setCategory("PANTRY");setUnit("g");setSuggestions([]);await saved(added)}finally{setSaving(false)}}}>
  <div className="pantry-add-heading"><b>AÃ±adir algo que ya tienes</b><small>Busca una variante existente o escribe tu nombre exacto.</small></div>
  <div className="pantry-name-picker"><input required value={name} onFocus={()=>setOpen(true)} onChange={event=>{setName(event.target.value);setOpen(true)}} placeholder="Ej. Pollo o hummus envasado" autoComplete="off" aria-label="Nombre del alimento"/>
   {open&&name.trim().length>=2&&<div className="pantry-suggestions">
    {suggestions.map(item=><button type="button" key={`${item.name}-${item.unit}`} onClick={()=>choose(item)}><span><b>{item.name}</b><small>{item.source==="CATALOG"?"Ya existe en tu catÃ¡logo":"Variante sugerida"}</small></span><em>{item.unit}</em></button>)}
    <button type="button" className="pantry-custom" onClick={()=>setOpen(false)}><span><b>Usar â€œ{name.trim()}â€</b><small>Guardar como nombre personalizado</small></span></button>
   </div>}
  </div>
  <select value={category} onChange={event=>setCategory(event.target.value)} aria-label="CategorÃ­a"><option value="PANTRY">Despensa</option><option value="FRUIT_VEGETABLES">Fruta y verdura</option><option value="MEAT_FISH">Carnes y pescados</option><option value="EGGS">Huevos</option><option value="DAIRY">LÃ¡cteos</option><option value="CEREALS_LEGUMES">Cereales y legumbres</option><option value="FROZEN">Congelados</option><option value="DRINKS">Bebidas</option><option value="OTHER">Otros</option></select>
  ...
  <button className="primary" disabled={saving}>{saving?"Guardandoâ€¦":"AÃ±adir"}</button>
 </form>
}
