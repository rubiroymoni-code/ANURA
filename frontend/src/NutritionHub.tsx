  ...
  return (
    <div className="overlay">
      <section className="modal nutrition-hub">
        ...
        {section === "cook" && <CookToday planId={activePlan?.id} meals={todayMeals} recipes={recipes} open={(recipeId,mealId)=>{setSelectedRecipe(recipeId);setSelectedMeal(mealId);setSection("recipe")}} />}
...
function CookToday({planId,meals,recipes,open}:{planId?:string;meals:TodayMeal[];recipes:Array<{id:string;name:string}>;open:(recipeId:string,mealId:string)=>void}){return <div className="cook-workspace"><BatchCooking planId={planId}/><section className="cook-today"><div className="food-pref-hero"><ChefHat/><span><small>COCINA DEL DÍA</small><h3>Preparaciones de hoy</h3><p>Abre una receta para ver el reparto exacto de esta comida.</p></span></div>{meals.length?meals.map(meal=>{const recipe=recipes.find(item=>item.name.trim().toLocaleLowerCase("es")===meal.recipe.trim().toLocaleLowerCase("es"));return <button key={meal.planned_meal_id} disabled={!recipe} onClick={()=>recipe&&open(recipe.id,meal.planned_meal_id)}><span><small>{meal.meal_type}</small><b>{meal.recipe}</b><em>{Number(meal.calories||0).toFixed(0)} kcal para ti</em></span><strong>Ver reparto →</strong></button>}):<div className="empty">No hay preparaciones asignadas para hoy.</div>}</section></div>}
