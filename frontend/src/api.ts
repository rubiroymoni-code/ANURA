...
export type WorkoutSummary = {id:string;name:string;date:string;status:string;workoutPlanDayId?:string;durationSeconds?:number;globalRpe?:number;exercises:number;sets:number;volume:number};
...
export type TodayMeal = { planned_meal_id:string;plan_id:string;plan_name:string;version:number;day_name:string;meal_type:string;meal_name:string;recipe:string;calories:number;protein:number;carbohydrates:number;fat:number;planned_calories:number;planned_protein:number;planned_carbohydrates:number;planned_fat:number;portion_multiplier:number;status:MealStatus;meal_date?:string;consumed_meal_id?:string;custom_name?:string;actual_portion?:string;notes?:string;completed_at?:string };
