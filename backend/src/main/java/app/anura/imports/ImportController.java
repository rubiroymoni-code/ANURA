...
public class ImportController {
 private final CsvSchemaRegistry schemas; private final TrainingPlanImportService imports; private final OnboardingEmailService onboarding;
 ImportController(CsvSchemaRegistry schemas,TrainingPlanImportService imports,OnboardingEmailService onboarding){this.schemas=schemas;this.imports=imports;this.onboarding=onboarding;}
 ...
 @PostMapping("/imports/{id}/confirm") Confirmed confirm(@PathVariable UUID id){Confirmed result=imports.confirm(id);onboarding.sendIfReady(CurrentUser.id());return result;}
