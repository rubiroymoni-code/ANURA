...
public class WorkoutExecutionService {
    ...
    @Transactional public SessionView transition(UUID id,String target,String event) {
        ...
        db.update("UPDATE workout_session SET status=?,last_activity_at=CURRENT_TIMESTAMP,updated_at=CURRENT_TIMESTAMP,version=version+1 WHERE id=? AND user_id=?",target,id,CurrentUser.id()); audit(event,"WORKOUT_SESSION",id,"SUCCESS"); return view(id);
    }
    ...
    @Transactional public SessionView complete(UUID id, CompleteRequest r) {
        ...
        db.update("UPDATE workout_session SET status='COMPLETED',completed_at=CURRENT_TIMESTAMP,last_activity_at=CURRENT_TIMESTAMP,duration_seconds=EXTRACT(EPOCH FROM (CURRENT_TIMESTAMP-started_at))::integer,global_rpe=?,energy_level=?,pump_level=?,pain_level=?,difficulty_level=?,notes=?,updated_at=CURRENT_TIMESTAMP,version=version+1 WHERE id=? AND user_id=?",r.globalRpe(),r.energyLevel(),r.pumpLevel(),r.painLevel(),r.difficultyLevel(),r.notes(),id,CurrentUser.id());
    ...
    private SyncResult syncOne(UUID sessionId,SyncOperation op) {
        ...
        try {
            ...
            else if("COMPLETE_SESSION".equals(op.operationType())) { JsonNode p=op.payload(); complete(sessionId,new CompleteRequest(decimal(p,"globalRpe"),integer(p,"energyLevel"),integer(p,"pumpLevel"),integer(p,"painLevel"),integer(p,"difficultyLevel"),text(p,"notes"))); entity=sessionId; }
    ...
    public record CompleteRequest(BigDecimal globalRpe,Integer energyLevel,Integer pumpLevel,Integer painLevel,Integer difficultyLevel,String notes){}
