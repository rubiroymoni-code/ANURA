...
public class WorkoutPlanService {
 ...
 public List<PlanExercise> details(UUID id){one(id);return db.query("SELECT d.week_number,d.day_number,d.day_name,d.session_name,pe.exercise_order,e.name,e.muscle_group,e.equipment,pe.sets,pe.reps_min,pe.reps_max,pe.target_rir,pe.target_rpe,pe.rest_seconds,pe.tempo,pe.warmup_required,pe.superset_group,pe.alternative_exercise_code,pe.instructions,pe.notes,d.id FROM workout_plan_day d JOIN planned_exercise pe ON pe.workout_plan_day_id=d.id JOIN exercise e ON e.id=pe.exercise_id WHERE d.workout_plan_id=? ORDER BY d.week_number,d.day_order,pe.exercise_order",(r,n)->new PlanExercise(r.getInt(1),r.getInt(2),r.getString(3),r.getString(4),r.getInt(5),r.getString(6),r.getString(7),r.getString(8),r.getInt(9),r.getInt(10),r.getInt(11),r.getBigDecimal(12),r.getBigDecimal(13),(Integer)r.getObject(14),r.getString(15),r.getBoolean(16),r.getString(17),r.getString(18),r.getString(19),r.getString(20),r.getObject(21,UUID.class)),id);}
