...
public class NutritionController {
  ...
  Map<String,Object> dashboard() {
    ...
    Map<String,Object> target=new LinkedHashMap<>(db.queryForList("SELECT calories,protein,carbohydrates,fat,fiber FROM nutrition_target WHERE user_id=? AND valid_from<=CURRENT_DATE ORDER BY valid_from DESC LIMIT 1",user).stream().findFirst().orElse(Map.of()));
    Map<String,Object> planned=db.queryForList("SELECT COALESCE(SUM(ump.calories),0) calories,COALESCE(SUM(ump.protein),0) protein,COALESCE(SUM(ump.carbohydrates),0) carbohydrates,COALESCE(SUM(ump.fat),0) fat FROM nutrition_plan p LEFT JOIN household_member access ON access.household_id=p.household_id JOIN nutrition_plan_day d ON d.nutrition_plan_id=p.id JOIN planned_meal pm ON pm.nutrition_plan_day_id=d.id JOIN user_meal_portion ump ON ump.planned_meal_id=pm.id AND ump.user_id=? AND COALESCE(ump.calories,0)>0 WHERE p.status='ACTIVE' AND (p.owner_id=? OR access.user_id=?) AND pm.is_default_option AND d.day_number=EXTRACT(ISODOW FROM CURRENT_DATE)::int AND d.week_number=(MOD(GREATEST((CURRENT_DATE-COALESCE(p.valid_from,p.created_at::date))/7,0),(SELECT MAX(cycle_day.week_number) FROM nutrition_plan_day cycle_day WHERE cycle_day.nutrition_plan_id=p.id))+1)",user,user,user).getFirst();
    if(((Number)planned.get("calories")).doubleValue()>0){
      for(String macro:List.of("calories","protein","carbohydrates","fat"))target.put(macro,planned.get(macro));
    }
  ...
  @GetMapping("/today")
  List<Map<String, Object>> today(@RequestParam(required=false) LocalDate date) {
    ...
    return db.queryForList(
        "SELECT pm.id planned_meal_id,p.id plan_id,p.name plan_name,p.version,d.day_name,pm.meal_type,pm.meal_name,r.name recipe,"
            ...
            + " JOIN recipe r ON r.id=pm.recipe_id JOIN user_meal_portion ump ON ump.planned_meal_id=pm.id AND ump.user_id=? AND COALESCE(ump.calories,0)>0"
            + " LEFT JOIN meal_option_selection choice ON choice.user_id=? AND choice.meal_date=? AND choice.option_group=pm.option_group"
            + " LEFT JOIN LATERAL (SELECT tracked.* FROM consumed_meal tracked LEFT JOIN planned_meal tracked_pm ON tracked_pm.id=tracked.planned_meal_id WHERE tracked.user_id=? AND tracked.meal_date=? AND (tracked.planned_meal_id=pm.id OR (tracked.planned_meal_id IS NOT NULL AND tracked_pm.meal_order=pm.meal_order)) ORDER BY (tracked.planned_meal_id=pm.id) DESC,tracked.completed_at DESC NULLS LAST,tracked.updated_at DESC LIMIT 1) cm ON TRUE"
  ...
  void selectMealOption(@PathVariable UUID mealId,@RequestBody MealOptionSelection input){
    ...
    UUID selectedUser=input.userId()==null?CurrentUser.id():input.userId();
    Map<String,Object> meal=plannedMeal(mealId);
    Integer member=db.queryForObject("SELECT COUNT(*) FROM nutrition_plan p JOIN nutrition_plan_day d ON d.nutrition_plan_id=p.id JOIN planned_meal pm ON pm.nutrition_plan_day_id=d.id WHERE pm.id=? AND (p.owner_id=? OR EXISTS(SELECT 1 FROM household_member hm WHERE hm.household_id=p.household_id AND hm.user_id=?))",Integer.class,mealId,selectedUser,selectedUser);
    if(member==null||member==0)throw new ApiException(HttpStatus.BAD_REQUEST,"OPTION_USER_NOT_IN_PLAN","Esa persona no forma parte de este plan");
    String group=(String)db.queryForObject("SELECT option_group FROM planned_meal WHERE id=?",String.class,mealId);
    Integer available=db.queryForObject("SELECT COUNT(*) FROM planned_meal pm JOIN user_meal_portion ump ON ump.planned_meal_id=pm.id WHERE pm.nutrition_plan_day_id=(SELECT nutrition_plan_day_id FROM planned_meal WHERE id=?) AND pm.option_group=? AND pm.option_code=? AND ump.user_id=? AND COALESCE(ump.calories,0)>0",Integer.class,mealId,group,input.optionCode(),selectedUser);
    ...
    db.update("INSERT INTO meal_option_selection(user_id,meal_date,option_group,option_code) VALUES(?,?,?,?) ON CONFLICT(user_id,meal_date,option_group) DO UPDATE SET option_code=EXCLUDED.option_code",selectedUser,date,group,input.optionCode());
  }

  @GetMapping("/today/{mealId}/options")
  List<Map<String,Object>> mealOptions(@PathVariable UUID mealId,@RequestParam(required=false) UUID userId){
    plannedMeal(mealId);
    UUID selectedUser=userId==null?CurrentUser.id():userId;
    return db.queryForList("SELECT pm.id planned_meal_id,pm.option_code,COALESCE(pm.option_label,pm.option_code) option_label,r.name recipe,ump.calories,ump.protein,ump.carbohydrates,ump.fat FROM planned_meal pm JOIN recipe r ON r.id=pm.recipe_id JOIN user_meal_portion ump ON ump.planned_meal_id=pm.id AND ump.user_id=? AND COALESCE(ump.calories,0)>0 WHERE pm.nutrition_plan_day_id=(SELECT nutrition_plan_day_id FROM planned_meal WHERE id=?) AND pm.option_group=(SELECT option_group FROM planned_meal WHERE id=?) ORDER BY pm.is_default_option DESC,pm.option_code",selectedUser,mealId,mealId);
  }

  @PostMapping("/today/{mealId}/complete")
  @Transactional
  Map<String,Object> completeTodayMeal(@PathVariable UUID mealId,@RequestParam(required=false) LocalDate date) {
    Map<String,Object> meal=plannedMeal(mealId);
    return savePlanned(mealId,"COMPLETED",null,meal,null,date==null?LocalDate.now():date);
  }

  @DeleteMapping("/today/{mealId}/completion")
  @Transactional
  void undoTodayMeal(@PathVariable UUID mealId,@RequestParam(required=false) LocalDate date){
    Map<String,Object> meal=plannedMeal(mealId);
    db.update("DELETE FROM consumed_meal cm USING planned_meal tracked WHERE cm.planned_meal_id=tracked.id AND cm.user_id=? AND cm.meal_date=? AND tracked.meal_order=?",CurrentUser.id(),date==null?LocalDate.now():date,meal.get("meal_order"));
  }
  ...
  Map<String,Object> adherence(@RequestParam(defaultValue="28") int days) {
    ...
    Integer expectedMeals=db.queryForObject("SELECT COUNT(*) FROM generate_series(CURRENT_DATE-?,CURRENT_DATE,INTERVAL '1 day') AS calendar(day) JOIN nutrition_plan p ON p.status='ACTIVE' JOIN nutrition_plan_day d ON d.nutrition_plan_id=p.id AND d.day_number=EXTRACT(ISODOW FROM calendar.day) AND d.week_number=(MOD(GREATEST((calendar.day::date-COALESCE(p.valid_from,p.created_at::date))/7,0),(SELECT MAX(cycle_day.week_number) FROM nutrition_plan_day cycle_day WHERE cycle_day.nutrition_plan_id=p.id))+1) JOIN planned_meal pm ON pm.nutrition_plan_day_id=d.id JOIN user_meal_portion ump ON ump.planned_meal_id=pm.id AND ump.user_id=? WHERE (p.owner_id=? OR EXISTS(SELECT 1 FROM household_member hm WHERE hm.household_id=p.household_id AND hm.user_id=?)) AND NOT EXISTS(SELECT 1 FROM nutrition_travel_mode t JOIN household_member tm ON tm.household_id=t.household_id WHERE tm.user_id=? AND t.status='ACTIVE' AND calendar.day::date BETWEEN t.start_date AND t.end_date AND t.exclude_from_adherence)",Integer.class,period-1,user,user,user,user);
    Integer expectedWorkouts=db.queryForObject("SELECT COUNT(*) FROM generate_series(CURRENT_DATE-?,CURRENT_DATE,INTERVAL '1 day') AS calendar(day) JOIN workout_plan p ON p.user_id=? AND p.status='ACTIVE' JOIN workout_plan_day d ON d.workout_plan_id=p.id AND d.day_number=EXTRACT(ISODOW FROM calendar.day)::int WHERE calendar.day::date>=COALESCE(p.valid_from,p.activated_at::date,p.created_at::date) AND (p.valid_until IS NULL OR calendar.day::date<=p.valid_until) AND (p.valid_from IS NULL OR d.week_number=(MOD(GREATEST((calendar.day::date-p.valid_from)/7,0),(SELECT MAX(cycle_day.week_number) FROM workout_plan_day cycle_day WHERE cycle_day.workout_plan_id=p.id))+1)) AND NOT EXISTS(SELECT 1 FROM workout_day_adjustment adjustment WHERE adjustment.user_id=p.user_id AND adjustment.workout_plan_day_id=d.id AND adjustment.original_date=calendar.day::date AND adjustment.status IN ('MOVED','SKIPPED'))",Integer.class,period-1,user);
    ...
    int weekWorkouts=db.queryForObject("SELECT COUNT(*) FROM generate_series(date_trunc('week',CURRENT_DATE),date_trunc('week',CURRENT_DATE)+INTERVAL '6 days',INTERVAL '1 day') AS calendar(day) JOIN workout_plan p ON p.user_id=? AND p.status='ACTIVE' JOIN workout_plan_day d ON d.workout_plan_id=p.id AND d.day_number=EXTRACT(ISODOW FROM calendar.day)::int WHERE calendar.day::date>=COALESCE(p.valid_from,p.activated_at::date,p.created_at::date) AND (p.valid_until IS NULL OR calendar.day::date<=p.valid_until) AND (p.valid_from IS NULL OR d.week_number=(MOD(GREATEST((calendar.day::date-p.valid_from)/7,0),(SELECT MAX(cycle_day.week_number) FROM workout_plan_day cycle_day WHERE cycle_day.workout_plan_id=p.id))+1)) AND NOT EXISTS(SELECT 1 FROM workout_day_adjustment adjustment WHERE adjustment.user_id=p.user_id AND adjustment.workout_plan_day_id=d.id AND adjustment.original_date=calendar.day::date AND adjustment.status IN ('MOVED','SKIPPED'))",Integer.class,user);
  ...
  private Map<String,Object> plannedMeal(UUID mealId){UUID user=CurrentUser.id();return db.queryForList("SELECT pm.id,pm.meal_order,pm.meal_type,pm.meal_name,ump.calories,ump.protein,ump.carbohydrates,ump.fat FROM planned_meal pm JOIN nutrition_plan_day d ON d.id=pm.nutrition_plan_day_id JOIN nutrition_plan p ON p.id=d.nutrition_plan_id LEFT JOIN household_member access ON access.household_id=p.household_id JOIN user_meal_portion ump ON ump.planned_meal_id=pm.id AND ump.user_id=? WHERE pm.id=? AND p.status='ACTIVE' AND (p.owner_id=? OR access.user_id=?)",user,mealId,user,user).stream().findFirst().orElseThrow(()->new ApiException(HttpStatus.NOT_FOUND,"MEAL_NOT_FOUND","Comida planificada no encontrada"));}
  private Map<String,Object> savePlanned(UUID mealId,String status,MealInput input,Map<String,Object> meal,String customName){return savePlanned(mealId,status,input,meal,customName,LocalDate.now());}
  private Map<String,Object> savePlanned(UUID mealId,String status,MealInput input,Map<String,Object> meal,String customName,LocalDate mealDate){UUID id=UUID.randomUUID();boolean substituted="SUBSTITUTED".equals(status);if(substituted&&(input==null||input.calories()==null))throw new ApiException(HttpStatus.BAD_REQUEST,"SUBSTITUTION_CALORIES_REQUIRED","Indica o estima las calorías de lo que has comido");Object calories=substituted?input.calories():input!=null&&input.calories()!=null?input.calories():meal.get("calories");Object protein=substituted?input.protein():input!=null&&input.protein()!=null?input.protein():meal.get("protein");Object carbs=substituted?input.carbohydrates():input!=null&&input.carbohydrates()!=null?input.carbohydrates():meal.get("carbohydrates");Object fat=substituted?input.fat():input!=null&&input.fat()!=null?input.fat():meal.get("fat");db.update("DELETE FROM consumed_meal cm USING planned_meal tracked WHERE cm.planned_meal_id=tracked.id AND cm.user_id=? AND cm.meal_date=? AND tracked.meal_order=? AND cm.planned_meal_id<>?",CurrentUser.id(),mealDate,meal.get("meal_order"),mealId);db.update("INSERT INTO consumed_meal(id,user_id,planned_meal_id,meal_date,meal_type,status,custom_name,portion,calories,protein,carbohydrates,fat,notes,completed_at) VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,CURRENT_TIMESTAMP) ON CONFLICT(user_id,planned_meal_id,meal_date) WHERE planned_meal_id IS NOT NULL DO UPDATE SET status=EXCLUDED.status,custom_name=EXCLUDED.custom_name,portion=EXCLUDED.portion,calories=EXCLUDED.calories,protein=EXCLUDED.protein,carbohydrates=EXCLUDED.carbohydrates,fat=EXCLUDED.fat,notes=EXCLUDED.notes,completed_at=CURRENT_TIMESTAMP,updated_at=CURRENT_TIMESTAMP",id,CurrentUser.id(),mealId,mealDate,normalizeMealType(meal.get("meal_type").toString()),status,customName,input==null?null:clean(input.portion()),"SKIPPED".equals(status)?null:calories,"SKIPPED".equals(status)?null:protein,"SKIPPED".equals(status)?null:carbs,"SKIPPED".equals(status)?null:fat,input==null?null:clean(input.notes()));return db.queryForMap("SELECT id,planned_meal_id,status,custom_name,calories,protein,carbohydrates,fat,notes,completed_at FROM consumed_meal WHERE user_id=? AND planned_meal_id=? AND meal_date=?",CurrentUser.id(),mealId,mealDate);}
  ...
  record MealOptionSelection(String optionCode,LocalDate date,UUID userId){}

  @GetMapping("/plans/{id}/week")
  List<Map<String, Object>> week(
      @PathVariable UUID id, @RequestParam(required=false) Integer week) {
    return db.queryForList(
        "SELECT pm.id planned_meal_id,d.day_number,d.day_name,(COALESCE(p.valid_from,p.created_at::date)+((d.week_number-1)*7)+(d.day_number-1)) meal_date,pm.meal_order,pm.meal_type,pm.meal_name,r.id recipe_id,r.name recipe,u.id user_id,u.display_name,pm.option_group,pm.option_code,COALESCE(pm.option_label,pm.option_code) option_label,"
            ...
            + " u.id=ump.user_id LEFT JOIN meal_option_selection choice ON choice.user_id=u.id AND choice.meal_date=(COALESCE(p.valid_from,p.created_at::date)+((d.week_number-1)*7)+(d.day_number-1)) AND choice.option_group=pm.option_group WHERE p.id=? AND d.week_number=COALESCE(?,(SELECT MIN(dx.week_number) FROM nutrition_plan_day dx WHERE dx.nutrition_plan_id=p.id)) AND COALESCE(ump.calories,0)>0 AND pm.option_code=COALESCE(choice.option_code,(SELECT fallback.option_code FROM planned_meal fallback WHERE fallback.nutrition_plan_day_id=pm.nutrition_plan_day_id AND fallback.meal_order=pm.meal_order AND fallback.option_group=pm.option_group AND fallback.is_default_option ORDER BY fallback.option_code LIMIT 1)) AND (p.owner_id=? OR"
  ...
  @GetMapping("/plans/{id}/meal-options")
  List<Map<String,Object>> planMealOptions(@PathVariable UUID id){
    UUID user=CurrentUser.id();
    return db.queryForList(
        "SELECT d.week_number,d.day_number,d.day_name,(COALESCE(p.valid_from,p.created_at::date)+((d.week_number-1)*7)+(d.day_number-1)) meal_date,pm.id planned_meal_id,pm.meal_order,pm.meal_type,pm.option_group,pm.option_code,COALESCE(pm.option_label,pm.option_code) option_label,r.name recipe,u.id user_id,u.display_name,ump.calories,(pm.option_code=COALESCE(choice.option_code,(SELECT fallback.option_code FROM planned_meal fallback WHERE fallback.nutrition_plan_day_id=pm.nutrition_plan_day_id AND fallback.meal_order=pm.meal_order AND fallback.option_group=pm.option_group AND fallback.is_default_option ORDER BY fallback.option_code LIMIT 1))) selected"
            + " FROM nutrition_plan p JOIN nutrition_plan_day d ON d.nutrition_plan_id=p.id JOIN planned_meal pm ON pm.nutrition_plan_day_id=d.id JOIN recipe r ON r.id=pm.recipe_id JOIN user_meal_portion ump ON ump.planned_meal_id=pm.id AND COALESCE(ump.calories,0)>0 JOIN app_user u ON u.id=ump.user_id LEFT JOIN meal_option_selection choice ON choice.user_id=u.id AND choice.meal_date=(COALESCE(p.valid_from,p.created_at::date)+((d.week_number-1)*7)+(d.day_number-1)) AND choice.option_group=pm.option_group"
            + " WHERE p.id=? AND (p.owner_id=? OR EXISTS(SELECT 1 FROM household_member hm WHERE hm.household_id=p.household_id AND hm.user_id=?)) ORDER BY d.week_number,d.day_order,pm.meal_order,u.display_name,pm.is_default_option DESC,pm.option_code",
        id,user,user);
  }
    ...
    List<Map<String, Object>> totals =
        db.queryForList(
            ...
            WITH portions AS (
              ...
              LEFT JOIN meal_option_selection choice ON choice.user_id=uip.user_id AND choice.meal_date=(COALESCE(p.valid_from,p.created_at::date)+((d.week_number-1)*7)+(d.day_number-1)) AND choice.option_group=pm.option_group
              WHERE d.nutrition_plan_id=? AND d.week_number=? AND pm.option_code=COALESCE(choice.option_code,(SELECT fallback.option_code FROM planned_meal fallback WHERE fallback.nutrition_plan_day_id=pm.nutrition_plan_day_id AND fallback.meal_order=pm.meal_order AND fallback.option_group=pm.option_group AND fallback.is_default_option ORDER BY fallback.option_code LIMIT 1))
              ...
              LEFT JOIN meal_option_selection choice ON choice.user_id=ump.user_id AND choice.meal_date=(COALESCE(p.valid_from,p.created_at::date)+((d.week_number-1)*7)+(d.day_number-1)) AND choice.option_group=pm.option_group
              WHERE d.nutrition_plan_id=? AND d.week_number=? AND pm.option_code=COALESCE(choice.option_code,(SELECT fallback.option_code FROM planned_meal fallback WHERE fallback.nutrition_plan_day_id=pm.nutrition_plan_day_id AND fallback.meal_order=pm.meal_order AND fallback.option_group=pm.option_group AND fallback.is_default_option ORDER BY fallback.option_code LIMIT 1))
                AND NOT EXISTS (SELECT 1 FROM nutrition_travel_mode t WHERE t.household_id=? AND t.status='ACTIVE' AND t.exclude_from_shopping AND (date_trunc('week',CURRENT_DATE)::date+d.day_number-1) BETWEEN t.start_date AND t.end_date)
                AND NOT EXISTS (
