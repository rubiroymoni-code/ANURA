...
public class NutritionController {
  ...
  Map<String,Object> adherence(@RequestParam(defaultValue="28") int days) {
    ...
    Map<String,Object> workouts=db.queryForMap("SELECT COUNT(*) FILTER(WHERE status='COMPLETED') completed,COUNT(*) FILTER(WHERE status='ABANDONED') abandoned,ROUND(COALESCE(AVG(CASE WHEN status='COMPLETED' THEN 100 WHEN status='ABANDONED' THEN 0 ELSE NULL END),0),1) score FROM workout_session WHERE user_id=? AND planned_date>=CURRENT_DATE-? AND deleted_at IS NULL",user,period-1);
    ...
    long workoutRecorded=((Number)workouts.get("completed")).longValue()+((Number)workouts.get("abandoned")).longValue();
    ...
    workouts.put("expected",workoutExpected);workouts.put("missing",Math.max(0,workoutExpected-workoutRecorded));workouts.put("score",workoutExpected==0?0:java.math.BigDecimal.valueOf(((Number)workouts.get("completed")).longValue()*100L).divide(java.math.BigDecimal.valueOf(workoutExpected),1,java.math.RoundingMode.HALF_UP));
    ...
    Map<String,Object> result=new LinkedHashMap<>();result.put("days",period);result.put("meals",meals);result.put("workouts",workouts);result.put("patterns",patterns);return result;
  }
