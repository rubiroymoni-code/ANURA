...
public class WebPushController {
  ...
  @PostMapping("/rest-timers") void scheduleRest(@RequestBody RestTimer input){if(input==null||input.timerId()==null||input.endAt()==null)throw new ApiException(HttpStatus.BAD_REQUEST,"INVALID_REST_TIMER","El temporizador de descanso no es válido");Instant now=Instant.now();if(input.endAt().isBefore(now.minusSeconds(5))||input.endAt().isAfter(now.plusSeconds(3600)))throw new ApiException(HttpStatus.BAD_REQUEST,"INVALID_REST_TIMER_TIME","La hora del descanso no es válida");db.update("INSERT INTO web_push_rest_timer(id,user_id,client_timer_id,end_at,status) VALUES(?,?,?,?, 'PENDING') ON CONFLICT(user_id,client_timer_id) DO UPDATE SET end_at=EXCLUDED.end_at,status='PENDING',sent_at=NULL",UUID.randomUUID(),CurrentUser.id(),input.timerId(),input.endAt());}
  ...
  private static String clean(String value){return value==null||value.isBlank()?null:value.trim();}record Subscription(String endpoint,String p256dh,String auth,String deviceName){}record Endpoint(String endpoint){}record RestTimer(UUID timerId,Instant endAt){}
}
