...
public class MonthlySummaryService {
  ...
  public void send(UUID user,boolean scheduled){
    ...
    Integer workouts=db.queryForObject("SELECT COUNT(*) FROM workout_session WHERE user_id=? AND status='COMPLETED' AND deleted_at IS NULL AND planned_date>=CURRENT_DATE-INTERVAL '30 days'",Integer.class,user);
